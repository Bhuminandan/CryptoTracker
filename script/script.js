...
mobThemeBtn.addEventListener("change", () => {
    document.body.classList.toggle("dark-mode");
...
mainHeaderRight.addEventListener("click", () => {
    ...
    async function getData() {
        try {let url = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=false&locale=en";
        let response = await fetch(url);
        let data = await response.json();
        createList(data);
       } catch(error){
          main.innerHTML = "";
          table.innerHTML = `<h4>Failed to fetch Due to ${error}, please try after some time!</h4>`
        }
    }
    getData()
    ...
    function createList(dataList) {
        dataList.forEach((singleData) => {
            ...
            let volumeDiv = tr.querySelector(".table-total__valume");
            let marketCapDiv = tr.querySelector(".table-market__cap");

            volumeDiv.style.color = "#fff";
            marketCapDiv.style.color = "#fff";
